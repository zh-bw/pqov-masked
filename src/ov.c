...
int ov_sign( uint8_t *signature, const sk_t *sk, const uint8_t *message, size_t mlen ) {
    ...
    // for (int i = 0; i < _SALT_BYTE; i++)
        // salt[i] = i+1;
    
    // printf ("\nSalt = ");
    // print_bitstring(salt, _SALT_BYTE);
    #if defined(_VALGRIND_)
    ...
    while ( MAX_ATTEMPT_VINEGAR > n_attempt ) {
        ...
        // printf("\nAttempt %d: vinegar = ", n_attempt);
        // print_bitstring(vinegar, _V_BYTE);
        #if defined(_VALGRIND_)
...
int ov_sign_shake256( uint8_t *signature, const sk_t *sk, const uint8_t *message, size_t mlen ) {
    // allocate temporary storage.
    uint8_t mat_l1[_O * _O_BYTE];
    uint8_t salt[_SALT_BYTE];
    randombytes( salt, _SALT_BYTE );
    ...
    uint8_t h_m_salt[mlen + _SALT_BYTE];
    memcpy(h_m_salt, message, mlen);
    memcpy(h_m_salt + mlen, salt, _SALT_BYTE);
    // The computation:  H(M||salt)  -->   y  --C-map-->   x   --T-->   w
    
    shake256(y, _PUB_M_BYTE, h_m_salt, mlen + _SALT_BYTE);  // H(M||salt)

    unsigned n_attempt = 0;
    while ( MAX_ATTEMPT_VINEGAR > n_attempt ) {
        uint8_t ctr = n_attempt & 0xff;                    // counter for generating vinegar
        n_attempt++;
        uint8_t h_vinegar[mlen + _SALT_BYTE + LEN_SKSEED + 1];
        memset(h_vinegar, 0, mlen + _SALT_BYTE + LEN_SKSEED + 1);
        memcpy(h_vinegar, h_m_salt, mlen + _SALT_BYTE);
        memcpy(h_vinegar + mlen + _SALT_BYTE, sk->sk_seed, LEN_SKSEED);
        h_vinegar[mlen + _SALT_BYTE + LEN_SKSEED] = ctr;  // H(M||salt||sk_seed||ctr ...
        shake256(vinegar, _V_BYTE, h_vinegar, mlen + _SALT_BYTE + LEN_SKSEED + 1);
        #if defined(_VALGRIND_)
        VALGRIND_MAKE_MEM_UNDEFINED(vinegar, _V_BYTE );  // mark secret data as undefined data
        #endif
