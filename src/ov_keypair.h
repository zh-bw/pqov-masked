...
#include "params.h"
#include "masking/masking_interface.h"
...
struct 
{
    unsigned char masked_sk_seed[LEN_SKSEED * N_SHARES]; ///< seed for generating secret key

    Masked masked_O[_V_BYTE * _O]; ///< T map, masked

    unsigned char P1[_PK_P1_BYTE]; ///< part of C-map, P1, also included in pk

    Masked masked_S[_PK_P2_BYTE]; ///< part of C-map, S, masked
} masked_sk_t;
